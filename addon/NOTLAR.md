# v7.52.2 — Tek Şimşek ve Toprak İzi kollara bağlandı

Kullanıcı sordu: *"tekli Şimşek hangi kola ekledin, nasıl
yapabiliyorum, onu göremedim."*

**Hiçbir kola eklememişim.** v7.52'de ikisi de yalnızca
"eşyasız" jest sırasındaydı.

## Neden görünmüyordu

`main.js` → `esyasizOyuncu()`:

```js
const sagKol = eldekiKol(oyuncu, "Mainhand");
const solKol = CIFT_EL_ACIK ? eldekiKol(oyuncu, "Offhand") : undefined;
...
if (secimler.length === 0) {          // <- YALNIZ boş elde
  const genel = sira[secimAl(id) % sira.length];
}
```

Elde bir kol varsa genel sıraya **hiç girilmiyor**. Yani Toprak
Kol tutan biri için iki yetenek de yoktu. Bulunamaması
kullanıcının gözünden kaçması değil, bağın eksikliğiydi.

## Ders

Bir yeteneğin **kayıtlı** olması ile **ulaşılabilir** olması
ayrı şeyler. `kayit.mjs` "hiçbir yetenek erişilemez değil"
diyordu ve doğru diyordu — eşyasız sırada gerçekten vardı.
Ama gerçek oyuncu elinde kolla dolaşıyor.

`test/tek_simsek.mjs` ve `test/toprak_izi.mjs` 0b bölümleri bunun
kaydı: bundan sonra bir yetenek kola bağlanmadan eklenirse test
düşecek.
...
| kol | eklenen | yeri |
|---|---|---|
| `pa:kol_toprak` (19 yetenek) | Tek Şimşek | Yön Şimşeği'nin **hemen ardında** (3.) |
| `pa:kol_toprak` | Toprak İzi | Toprak Duvar'ın **hemen ardında** (10.) |
| `pa:kol_kanli` (7 yetenek) | Tek Şimşek | Yön Şimşeği'nin ardında (6.) |

Yer seçimi tesadüf değil: Tek Şimşek nişan alıp şimşek atıyor,
Yön Şimşeği de — yan yana dursunlar. Toprak İzi toprak koyup
geri alıyor, Toprak Duvar da.

**Toprak İzi Kanlı Kol'a bilerek konmadı**: o bir dövüş kolu,
toprak işi Toprak Kol'un konusu. Test bunu da tutuyor.

İkisi eşyasız sırada da duruyor — boş elle de çalışıyorlar.

Mutasyon: bağı tamamen sildim, yalnız Kanlı Kol'dan sildim,
sırayı bozdum — üçü de yakalandı.
